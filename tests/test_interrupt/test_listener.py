...
class TestSigtermHandlerDelegation:
    ...
    def test_sigterm_handler_restores_process_baseline_after_listener_stop(
        self, listener: KeyboardListener
    ) -> None:
        # Requirement: SIGTERM during provider teardown restores the cached
        # process baseline even though listener.stop() cleared instance state.
        import signal as signal_module
        ...
        with (
            ...
            patch("signal.signal", side_effect=fake_signal),
            patch("conductor.interrupt.listener.restore_terminal_baseline") as restore_baseline,
        ):
            listener._register_cleanup_handlers()
            registered_handler = cast(Callable[[int, object], None], captured_handler[0][1])
            listener._original_settings = None

            registered_handler(signal_module.SIGTERM, None)

        restore_baseline.assert_called_once_with()
